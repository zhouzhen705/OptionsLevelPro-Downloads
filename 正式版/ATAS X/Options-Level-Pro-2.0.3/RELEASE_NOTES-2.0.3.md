# Release Notes / Options Level Pro 2.0.3 / ATAS X

This is a stable Options Level Pro 2.0.3 release package for ATAS X.
这是适用于 ATAS X 的 Options Level Pro 2.0.3 稳定发布包。

## Added / 新增

- Added local Options Level Pro data storage under ``C:\Program Files (x86)\ATAS Platform\olp`` for static levels and 0DTE heatmap snapshots.
- 新增 ``C:\Program Files (x86)\ATAS Platform\olp`` 本地数据目录，用于保存静态关键位和 0DTE 热图快照。
...
- Unified the ATAS X release-package version number at ``2.0.3``.
- ATAS X 发布包版本号统一为 ``2.0.3``。
...
- If the server static-level date is behind the current ET date, the indicator shows today's data as unavailable instead of updated.
- 当服务器静态关键位日期落后当前 ET 日期时，指标显示今日期权数据暂无数据，不再显示已更新。
